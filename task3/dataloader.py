...
#                 if '''not self.encode and''' not os.path.exists(pjoin(path, directory, "pre_encoded_" + str(img_size[0]))):
                if not os.path.exists(pjoin(path, city, "pre_encoded_" + str(img_size[0]))):
                    ...
                    print(" - Folders exist:", os.path.exists(pjoin(path, city, "pre_encoded_" + str(img_size[0]))) and os.path.exists(pjoin(self.root, "leftImg8bit", split, city, "pre_encoded_" + str(img_size[0]))))

                # iterate over all imaged from the city
                for i, image in enumerate(os.listdir(pjoin(path, city))):
                    if "color" in image:
                        lbl_path = pjoin(path, city, image)
                        self.files[split].append(pjoin(path, city, "pre_encoded_" + str(img_size[0]), image.replace("png", "pt")))
                        ...
                        if encode:
                            ...
                            img_save_path = pjoin(self.root, "leftImg8bit", split, city, "pre_encoded_" + str(img_size[0]), image.replace("png", "pt")).replace("gtFine", "leftImg8bit").replace("_color", "")
                            torch.save(img, img_save_path)
                            torch.save(lbl, self.files[split][-1])
                            print("\r\tFiles", i, "exist:", os.path.exists(img_save_path) and os.path.exists(self.files[split][-1]), end="")
                if encode:
                    print() 
...
d = cityscapesDataset("../datasets/data/cityscapes/", split="val")
'''
img, lbl = d[0]
